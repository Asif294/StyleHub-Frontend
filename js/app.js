const getallproduct =()=>{
    ...
    fetch("http://127.0.0.1:8000/product/product/")
    .then((res)=>res.json())
    .then((products)=>{
        ...
        products.forEach(product => {
            ...
            div.innerHTML=`
                    ...
                    <a hraf="#">
                    <h1 class="mt-2 p-2 rounded-2xl text-m bg-black text-white cursor-pointer text-center h-10 ">Add to card</h1>
                    </a>
    ...
    .then((data)=>{
        data.forEach((item)=>{
            ...
            div.innerHTML=`
                ...
                <ul class="mt- ">
                    <button  class="btn btn-secondary  m-1">${item.color}</button>
                 </ul>
            `
    ...
    .then((data)=>{
        data.forEach((item)=>{
            ...
            div.innerHTML=`
                ...
                <ul class="mt- ">
                    <button  class="btn btn-secondary  m-1">${item.size}</button>
                 </ul>
            `
             parent.appendChild(div);
